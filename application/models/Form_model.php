...
class Form_model extends CI_Model {
  ...
  function getformfield($fld_formid) {
    $gff = $this->db->query("select t0.*,t1.fld_querysql,t1.fld_queryp01,t1.fld_queryp02,t1.fld_queryid,t1.fld_querynm  from tbl_formfield t0
  ...
  function getsubform($fld_formid) {
    $gsf = $this->db->query("select t1.fld_formid,t1.fld_formlbl,t1.fld_formtmpl,t3.fld_querysql,t1.fld_formcreate,t1.fld_formdelete,
    t1.fld_formnm,t0.fld_formrelp,t0.fld_formrelc,t4.fld_tblnm,t4.fld_tblpkey,
    t0.fld_subformshow,t0.fld_subformty,t3.fld_queryid,t2.fld_viewnm,t0.fld_subformheader 
    from tbl_subform t0
  ...
  function getformview($fld_formid) {
    ...
    t0.fld_viewrelid,t2.fld_viewtmpl,t2.fld_viewcreate,t2.fld_viewnm,fld_viewupdate,fld_viewdelete,t0.fld_formviewheader  from tbl_formview t0
  ...
  function getfollowup($node,$groupid) {
    ...
    left join tbl_tyval t6 on t6.fld_tyvalcd=t0.fld_btstat and t6.fld_tyid=2
   left join tbl_tyval t7 on t7.fld_tyvalcd=t1.fld_workflowtoa and t7.fld_tyid=2
    where
    t0.fld_btid=$node 
    and 
    t6.fld_tyvalcfg >= t7.fld_tyvalcfg
    and
	...
	function getPrintLink($fid,$groupid)
	    {
 		$gsf = $this->db->query("select t0.fld_formprintnm,t0.fld_formprinturl,t0.fld_formprintsts 
                from tbl_formprint t0 
                where fld_formid=$fid
                and if(t0.fld_formprintauth = '',1,$groupid in (t0.fld_formprintauth)) ");
 		if ($gsf->num_rows() > 0)
 		{
 		return $gsf->result();
 		}
 ...
 /*
  function on getApprovalRole($btid,$groupid) {
	$sql="select t4.fld_aprvroleid
		   from tbl_bth t0 
	...
	$result->fld_aprvroleid;
	
    $sql="SELECT t0.fld_aprvtktstat
		from tbl_aprvtkt t0
		where t0.fld_btid=$btid AND t0.fld_aprvruleroleord=
		(select t0.fld_aprvruleroleord from 
		tbl_aprvtkt t0
		WHERE t0.fld_btid=$btid AND t0.fld_usergrpid=$groupid) -1";
	$query=$this->db->query($sql)->row();
	if ($query->num_rows() > 0)
	{
		if ($result->fld_aprvroleid < 2)
		{
			return $result->fld_aprvroleid
		} else {
			return 3;
		}
	} else {
  ...
  */
  ...
  function getApprovalRole($btid,$groupid) {
    $sql="select t4.fld_aprvroleid
	  ...
	  where t0.fld_btid=$btid and t4.fld_usergrpid=$groupid";
    $result = $this->db->query($sql)->row();
    $fld_aprvroleid=$result->fld_aprvroleid;
    $sql="SELECT t0.fld_aprvtktstat
          from tbl_aprvtkt t0
	  where t0.fld_btid=$btid AND t0.fld_aprvruleroleord=
	  (select t0.fld_aprvruleroleord from tbl_aprvtkt t0
          WHERE t0.fld_btid=$btid AND t0.fld_usergrpid=$groupid) -1";
    $query=$this->db->query($sql);
    if ($query->num_rows() > 0){
      foreach ($query->result() as $row){
        $status=$row->fld_aprvtktstat;	
      }
    } else {
      $status=2;
    }
    $sql="SELECT t0.fld_aprvtktstat
        ...
        where t0.fld_btid=$btid AND t0.fld_aprvruleroleord=
	(select t0.fld_aprvruleroleord from tbl_aprvtkt t0
        WHERE t0.fld_btid=$btid AND t0.fld_usergrpid=$groupid)";
    $query=$this->db->query($sql);
    if ($query->num_rows() > 0){
      foreach ($query->result() as $row){
        $status1=$row->fld_aprvtktstat;
      }
    } else {
      $status1=2;
    }
    $hasil=array(
      'fld_aprvroleid' => $fld_aprvroleid,
      'status' => $status,
      'status1' => $status1
    );
    //print_r ($hasil);
