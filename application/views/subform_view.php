...
if ($mode == 'edit' || $mode == 'add') {
  ...
  foreach ($subform as $rsubform) {
    if ($rsubform->fld_subformty == 1) {
      ...
      function copyValue" . $rsubform->fld_formnm . "(field){
        var tbl = document.getElementById('" . $rsubform->fld_formid . "');
        var x = tbl.rows.length - 1;
        var val = document.getElementById(field + 1).value;
        for (u = 2; u < x; u++) {
          document.getElementById(field + u).value = val;
        }
      ...
      function addRow" . $rsubform->fld_formnm . "(){
        ...
        $relfldval = (isset($rffval->$relfldp)) ? $rffval->$relfldp : '123ABC' ;
      ...
      $sfgvrs = $this->db->query("select * from $sftblnm t0 where t0.$relfldc = '$relfldval' order by 1");
      ...
      if($rsubform->fld_subformheader != '') {
        echo '<tr bgcolor="#CDCBBF" align="center">';
        echo '<td width="20" rowspan="2"><a href="javascript:addRow' . $rsubform->fld_formnm . '()' . '"><img src="' . base_url() . 'images/application_add.png" width="14" border=0 ></a></td> '.$rsubform->fld_subformheader.'';
        echo '</tr>';
      }
      else{
        ...
        foreach ($sfformfield as $rsfformfield) {
          if ($rsfformfield->fld_formfieldshow == 1) {
            if($rsfformfield->fld_formfieldmdtr == 1) {
              echo '<td align="center" nowrap><span style="color: rgb(255, 0, 0);">*</span>' . $rsfformfield->fld_formfieldlbl . '&nbsp;<a href="javascript:copyValue' . $rsubform->fld_formnm . '(\'' .  $rsubform->fld_formnm . $rsfformfield->fld_formfieldnm . '\')' . '"><span style="font-size:14px;">&#8794;</span></a></td>';
            } else {
              echo '<td align="center" nowrap>' . $rsfformfield->fld_formfieldlbl . '&nbsp;<a href="javascript:copyValue' . $rsubform->fld_formnm . '(\'' .  $rsubform->fld_formnm . $rsfformfield->fld_formfieldnm . '\')' . '"><span style="font-size:14px;">&#8794;</span></a></td>';
            }
          }
        }
        echo "</tr>";
...
<?
      ...
      foreach ($sfformfield as $rsfformfield) {
        ...
        $valuesf = $rsfffval->$ffname;
         ...
         if ($rsfformfield->fld_formfieldhideauth != ""){
          $grp = "$groupid,$group_add";
          $arr_group = explode(',', $grp);
          $x =  count($arr_group);
          $hide = 0;
          foreach ($arr_group as $value) {
          $hide_auth = preg_replace("/groupid/is",$value,$rsfformfield->fld_formfieldhideauth);
          ...
          if($hide > 0) {
          $rsfformfield->fld_formfieldshow = 0;
          }
          }
        ...
        if ($rsfformfield->fld_formfieldshow == 1) {
          ...
          $input_tag = array (
		    ...
		    break;
                    ...
                    case "money":
		     $value_dsc = '';
	             $value_dsc = number_format($valuesf,2,'.',',');
                     $input_tag = array (
		    ...
		    if($rsfformfield->fld_queryp01 == "") {
                      $fld_querysql = $rsfformfield->fld_querysql . " having id = '$valuesf'";
                    } else {
                      $fld_querysql = $rsfformfield->fld_querysql . " and $rsfformfield->fld_queryp01 = '$valuesf'";
                    }
			...
			if (substr($bindval,0,4) == 'fld_' || substr($bindval,0,4) == '@fld') {
			  $ctbind = $ctbind +1;
			  ${'bindfield' . $ctbind} = $bindname;
                          if(substr($bindval,0,4) == '@fld') {
                            $bindval = substr($bindval,1);
                            $bindval = $rsfffval->$bindval;
                          } else {
		    ...
		    break;
                    ...
                    echo '<a href="javascript:getnow(\'' . $rsubform->fld_formnm . $rsfformfield->fld_formfieldnm . ${$rsubform->fld_formid . 'ctval'} . '\')"><img src="' . base_url() .'/images/calendar.jpg" width="14" height="14" border="0"></a>';
                    echo "<script>
		    ...
		    if ($rsfformfield->fld_formfieldronly == 1) {
		      ...
		      $value_dsc = '';
		      $query = $this->db->query("$fld_querysql having id = '$valuesf'");
		      $lquery = $query->row();
		      if (count($lquery) > 0) {
			$value_dsc = $lquery->name;
		      }
		      ...
		      $input_tag['readonly'] = 1;
		      echo form_input($input_tag,'',$js_lookup);
                      echo '<input type="hidden" name="' . $rsfformfield->fld_formfieldnm . '" id="' . $rsfformfield->fld_formfieldnm . '" value="' . $value . '" />';
			...
			if ($this->input->get($bindname)) {
		      ...
		      $ddsize = $rsfformfield->fld_formfieldwidth;
		      if ($gopt->num_rows() > 0)
		      {
			$lopt = $gopt->result();
		      }
		      ...
		      foreach ($lopt as $ropt)
		      {
				  $options[$ropt->id]= $ropt->name;
                       ...
                       $rem = "id='" . $rsubform->fld_formnm . $rsfformfield->fld_formfieldnm . ${$rsubform->fld_formid . 'ctval'} . "' class='$classsf'";

		        echo form_dropdown($rsubform->fld_formnm . $rsfformfield->fld_formfieldnm . ${$rsubform->fld_formid . 'ctval'}, $options, $valuesf, $rem);
		      }
...
// $no = $no+1;
	...
	switch($rsfformfield->fld_formfieldtag) {
		   ...
		   ";
		    break;
                ...
                case "getnow":
                  ...
                  img.className = 'selectorButton';
                  newlink = document.createElement('a');
                  newlink.setAttribute('href', 'javascript:getnow(\"" . $rsubform->fld_formnm . $rsfformfield->fld_formfieldnm  ."' + iteration + '\")');
                  newlink.appendChild(img);
                  cell" . $rsfformfield->fld_formfieldnm . ".appendChild(newlink);
                   ";
		...
		$dbind = array();
		   ...
		   " . $opt . "
		   cell" . $rsfformfield->fld_formfieldnm . ".appendChild(el" . $rsfformfield->fld_formfieldnm . ");
                    var dd =  el" . $rsfformfield->fld_formfieldnm . ";
		  ...
		  newlink.setAttribute('href', 'javascript:void(0)');
		  newlink.setAttribute('onClick', 'popup_selector" . "(\'" . $rsubform->fld_formnm ."\'," . "' + iteration + '" . ",document.getElementById(\'" . $rsubform->fld_formnm . $rsfformfield->fld_formfieldnm  . "' + iteration + '_dsc" . "\').value,\'" . base_url() . "\',\'" . $rsfformfield->fld_querynm . "\',\'" . $rsubform->fld_formnm . $rsfformfield->fld_formfieldnm . "' + iteration + '\',\'$bindfield1\',\'$bindfield2\',\'$bindfield3\',\'$bindfield4\',\'$bindfield5\')');
		  newlink.appendChild(img);
		  cell" . $rsfformfield->fld_formfieldnm . ".appendChild(newlink);
...
if($rformview->fld_viewtmpl == "") {
  echo '<table bgcolor="#FFFFFF" cellpadding="0" cellspacing="1" width="100%" id="' . $rsubform->fld_formid . '">';
  if($rformview->fld_formviewheader == '') {
    echo '<tr height="10" bgcolor="#CDCBBF" align="center">';
    ...
    echo "$rformview->fld_formviewheader";
  }
  ...
  foreach ($fvgvrs as $rfvgvrs) {
    ...
    foreach ($fvgvcol as $rfvgvcol){
      ...
      if ($obj1 == 'crud') {
        echo '<td align="center" width="30" nowrap>';
	if ($rformview->fld_viewupdate == 1) {
	  echo '<a href="' . base_url() . 'index.php/page/form/' . $rformview->fld_viewnm . '/edit/' . $rfvgvrs->$rfvgvcol .'?' .  $relview . '='  . $node . '&sf=1"><img src="' . base_url() . 'images/application_edit.png" width="14" height="14" border=0 title="Edit Record"></a>';
	    }
	    if ($rformview->fld_viewdelete == 1) {
	      echo '<a href="' . base_url() . 'index.php/page/delete_process/' . $rformview->fld_viewnm . '/' . $rfvgvrs->$rfvgvcol .'" onclick= "return confirm(\' Are you sure want to delete this record ? \')"><img src="' . base_url() . 'images/application_delete.png" width="14" height="14" border=0  title="Delete Record"></a>';
	    }
	    echo '</td>';
	}
	else if ($obj2 == 'price')
